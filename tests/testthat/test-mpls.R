...
test_that("MPLS full-rank fails for r > d", {
  expect_error(alg(X=data$separable$X, Y=data$separable$Y, r=d+1))
})

test_that("MPLS full-rank r == d", {
  r <- d
  run_data_test(data, alg=alg, r)
})

test_that("MPLS full-rank r == K+1", {
  r <- K+1
  run_data_test(data, alg=alg, r)
})

test_that("MPLS full-rank r == K", {
  r <- K
  run_data_test(data, alg=alg, r)
})

test_that("MPLS full-rank r == 1", {
  r <- 1
...
test_that("MPLS low-rank fails for r > d", {
  expect_error(alg(X=data$separable$X, Y=data$separable$Y, r=d+1))
})

test_that("MPLS low-rank r == K+1", {
  r <- K+1
  run_data_test(data, alg=alg, r)
})

test_that("MPLS low-rank r == K", {
  r <- K
  run_data_test(data, alg=alg, r)
})

test_that("MPLS low-rank r == 1", {
  r <- 1
  run_data_test(data, alg=alg, r=r)
})
...
n <- 100
d <- 3
test_that("MPLS works with multi-class", {
