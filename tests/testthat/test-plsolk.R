...
test_that("PLSOLK full-rank fails for r > d", {
  expect_error(alg(data$separable$X, data$separable$Y, d+1))
})

test_that("PLSOLK full-rank r == d", {
  r <- d
  run_data_test(data, alg=alg, r)
})

test_that("PLSOLK full-rank r == K+1", {
  r <- K+1
  run_data_test(data, alg=alg, r)
})

test_that("PLSOLK full-rank r == K", {
  r <- K
  run_data_test(data, alg=alg, r)
})

test_that("PLSOLK full-rank r == 1", {
  r <- 1
...
test_that("PLSOLK low-rank fails for r > d", {
  expect_error(alg(data$separable$X, data$separable$Y, d+1))
})

test_that("PLSOLK low-rank r == K+1", {
  r <- K+1
  run_data_test(data, alg=alg, r)
})

test_that("PLSOLK low-rank r == K", {
  r <- K
  run_data_test(data, alg=alg, r)
})

test_that("PLSOLK low-rank r == 1", {
  r <- 1
  run_data_test(data, alg=alg, r=r)
})
...
n <- 100
d <- 3
test_that("PLSOLK works with multi-class", {
