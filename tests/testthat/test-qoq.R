...
run_data_test <- function(data, alg, r=NULL, cutoff=0.35, sep=TRUE, piled=FALSE){
  result <- lapply(data, function(dat) {
    ...
    class <- do.call(qda, list(embed$Xr, dat$dat$Y))
...
test_that("QOQ full-rank fails for r > d", {
  expect_error(alg(data$separable$dat$X, data$separable$dat$Y, d+1))
})

test_that("QOQ full-rank r == d", {
  r <- d
...
test_that("QOQ low-rank fails for r > d", {
  expect_error(alg(data$separable$dat$X, data$separable$dat$Y, d+1))
})

test_that("QOQ low-rank r == K+1", {
  r <- K+1
  run_data_test(data, alg=alg, r, cutoff=NULL)
})

test_that("QOQ low-rank r == K", {
  r <- K
  run_data_test(data, alg=alg, r, cutoff=NULL)
})

test_that("QOQ low-rank r == 1", {
  r <- 1
...
n <- 100
d <- 3
test_that("QOQ works with 3-class", {
