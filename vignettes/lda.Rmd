...
Projecting with LDA to $K-1=1$ dimensions and classifying the data:

```{r, fig.width=5}
result <- fs.classify.lda(X, Y, X, Y.test=Y)
...
ggplot(data, aes(x=x1, fill=y)) +
  ...
  xlab(TeX("$x_1$")) +
  ylab(TeX("Density")) +
  ggtitle(TeX(sprintf("Projected Data using LDA, $\\hat{L} = %.2f$", result$Lhat)))
```
