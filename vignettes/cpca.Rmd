...
Projecting with cPCA to 3 dimensions and visualizing the first 2:

```{r, fig.width=5}
result <- fs.project.cpca(X, Y, r)
...
ggplot(data, aes(x=x1, fill=y)) +
  ...
  xlab(TeX("$x_1$")) +
  ylab(TeX("Density")) +
  ggtitle(TeX(sprintf("$cPCA \\rightarrow LDA$, $\\hat{L} = %.2f$", lhat)))
```
...
```{r, fig.width=5}
testdat <- fs.sims.rtrunk(n, d, rotate=TRUE)
X <- testdat$X
Y <- testdat$Y
