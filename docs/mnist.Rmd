...
r <- 10
nplot <- 9
pca <- fs.project.pca(set1$X, set1$Y, r=r)
...
r <- 10
nplot <- 9
cpca <- fs.project.cpca(set1$X, set1$Y, r=r)
...
r <- 10
nplot <- 9
lol <- fs.project.lol(set1$X, set1$Y, r=r)
...
r <- 10
nplot <- 9
cca <- fs.project.lrcca(set1$X, set1$Y, r=r)
...
Below, we perform the afformentioned simulations with 10-fold validation:
  ...
  for (i in 1:length(algorithms)) {
    for (r in rs) {
      for (class in classalgs) {
        res <- suppressWarnings(fs.eval.xval(setX, setY, r=r, alg=algorithms[i][[1]], classifier=class, k=k))
