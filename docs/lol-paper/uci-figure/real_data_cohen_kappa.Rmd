---
title: "UCI/PMLB Analysis -- Cohen's Kappa"
...
lol.dat.prep <- lol.dat %>%
  ...
  dplyr::inner_join(lol.rc %>%
                    ...
                    by=c("Fold", "Dataset")) %>%
  # normalize
  mutate(Kappa=(Er.Rt - RC.Er.Rt)/(1 - RC.Er.Rt), Kappa.REF=(REF.Er.Rt - RC.Er.Rt)/(1 - RC.Er.Rt)) %>%
  # for each (Algorithm, Dataset) compute the mean normalized rstar, error rate
  # over folds
  dplyr::group_by(Algorithm, Dataset, n) %>%
  dplyr::summarize(Kappa.dif=mean(Kappa - Kappa.REF), Kappa=mean(Kappa), Er.Rt=mean(Er.Rt)) %>%
...
kappa.scatter <- function(data, title="") {
  data %>%
    ...
    ggplot(aes(x=Algorithm, y=Kappa.dif, color=Algorithm)) +
      ...
      annotate("text", size=4, label=sprintf("%s better", ref.alg), color="black", x=5, y=.35) +
      annotate("text", size=4, label=sprintf("%s worse", ref.alg), color="black", x=5, y=-.35) +
...
rank.hm <- function(data, title="") {
  ...
  data %>%
    ...
    ggplot(aes(x=Rank, y=Algorithm, fill=Count)) +
      geom_tile() +
      scale_fill_gradient(low="#FFFFFF", high="#660099", name="#Datasets",
                          breaks=c(0, 5, 10)) +
      ...
      geom_text(data=p.vals, aes(x=6.75, y=Algorithm, fill=NULL,
                                 label=sprintf("%.3f", p.value)), hjust=0) +
      annotate("text", label="p-value", x=6.75, hjust=0, y="LOL")
}
```

```{r}
grid.arrange(kappa.scatter(lol.dat.prep, "(A)"), rank.hm(lol.dat.prep, "(B)"), widths=c(0.6, 0.4), nrow=1)
```
