...
sims <- list(lol.sims.rtrunk, lol.sims.toep, lol.sims.rtrunk, lol.sims.fat_tails, lol.sims.rev_rtrunk, lol.sims.cross)
maxr <- c(30, 90, 30, 30, 30, 30)
ds <- c(100, 100, 100, 1000, 100, 100)
# additional arguments for each simulation scenario
opt_args <- list(list(), list(), list(K=3), list(rotate=TRUE), list(robust=n*.2), list())
sim_names = c("Trunk-2", "Toeplitz", "Trunk-3", "Fat-Tails (D=1000)", "Robust", "Cross")
sim_titles = c("(A)", "(B)", "(C)", "(D)", "(E)", "(F)")
ndim <- c(10, 10, 10, 10, 10, 10)
sim_min <- c(10, 30, 10, 10, 10, 10)
by <- c(10, 30, 10, 10, 10, 10)
...
for (i in 1:length(sim_names)) {
  ...
  if (i == 5) {
    ...
    data_sub <- data_sub[pt.dat$alg != "QOQ",]
