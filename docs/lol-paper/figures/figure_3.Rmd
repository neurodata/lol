...
sim_twod_scatter <- function(X, Y, title="", number="", nbreaks=4, outlier=NULL, d=c(1, 2)) {
  reorder <- c()
...
for (i in 1:length(sims)) {
  ...
  d <- dims[[i]]
  sim.dims[[counter]] <- sim_twod_scatter(simn$X, simn$Y, title=sim_names[i], number=sim_titles[i], outlier=simn$robust, d=d)
  counter <- counter + 1
}
```
...
algs <-  c("LOL/Q", "RLOL", "QOQ", "PLS", "CCA", "LDA", "PCA", "RP", "ROAD", "LASSO")
...
results.means$alg <- revalue(results.means$alg, c("LOL"="LOL/Q"))
...
sim_leg.plot <- ggplot(data=subset(results.means, !(alg  %in% c("ROAD", "LASSO"))), aes(x=r, y=lhat, color=alg, linetype=alg)) +
  geom_line(size=1.2) +
  scale_color_manual(values=acols, guide=guide_legend(ncol=4, byrow=TRUE), name="Algorithm") +
  scale_linetype_manual(values=linestyle, guide=guide_legend(ncol=4, byrow=TRUE), name="Algorithm") +
  theme(legend.position="bottom", axis.title.y=element_text(size=fsize)) +
  theme_bw()
...
sim_plots <- sapply(1:length(sim_plots), function(j) {
    ...
    if (j != 1) {
      ...
      resp <- resp + ylab("Misclassification Rate")
    }
    return(resp)
  }, simplify=FALSE)
...
sim.dims <- sapply(1:length(sim.dims), function(j) {
    resp <- sim.dims[[j]] + ggtitle("") + theme(legend.position=NaN)
    # remove the ylabel of only the non-left most columns
    if (j == 1) {
      resp <- resp + xlab("Magnitude")
    }
    return(resp)
  }, simplify=FALSE)

tfonts = 14
grid_sim <- grid.arrange(grid.arrange(grobs=sim_plots, nrow=nsim), sim_leg, nrow=2, heights=c(.95, .09),
                         top=textGrob("Classification Performance\n(n=100)", gp=gpar(fontsize=tfonts, face="bold")))
grid_mean <- grid.arrange(grid.arrange(grobs=mean_plots, nrow=nsim), mean_leg, nrow=2, heights=c(.95, .09),
                         top=textGrob("Means\n(First 10 Dimensions)", gp=gpar(fontsize=tfonts, face="bold")))
grid_cov <- grid.arrange(grid.arrange(grobs=cov_plots, nrow=nsim), cov_leg, nrow=2, heights=c(.95, .09),
                         top=textGrob("Covariances\n(First 10 Dimensions)", gp=gpar(fontsize=tfonts, face="bold")))
grid_simfill <- grid.arrange(grid.arrange(grobs=sim.fill, nrow=nsim), simfill_leg, nrow=2, heights=c(.95, .09),
                         top=textGrob("Generative Model\n(p=100)", gp=gpar(fontsize=tfonts, face="bold")))
grid_simdim <- grid.arrange(grid.arrange(grobs=sim.dims, nrow=nsim), simfill_leg, nrow=2, heights=c(.95, .09),
                         top=textGrob("Simulated Data\n(p=100)", gp=gpar(fontsize=tfonts, face="bold")))
```

We combine and plot:

```{r, fig.height=11, fig.width=10}
grid.arrange(grid_simdim, grid_sim, ncol=2, widths=c(0.35, 0.35))
```
