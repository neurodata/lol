...
mcols <- c("#808080", "#f41711", "#0e3ec1", "#8B4513")
...
plot_sim_lhats <- function(data, cols, pt.dat, linestyle, title="", by=10, from=10, ylab=TeX("$\\hat{L}$"),
  ...
  plot_sims <- ggplot(data, aes(x=r, y=lhat, linetype=alg, color=alg)) +
    geom_line(size=.95) +
    scale_color_manual(values=cols, guide=guide_legend(nrow=3, byrow=TRUE, override.aes=list(shape=NA)), name="Algorithm") +
    scale_linetype_manual(values=linestyle, guide=guide_legend(nrow=3, byrow=TRUE), name="Algorithm") +
...
sim_twod_scatter <- function(X, Y, title="", ylab="Dimension 1", xlab="Dimension 2", ndim=10, nbreaks=4) {
  reorder <- c()
  for (ylab in unique(Y)) {
    reorder <- c(reorder, sample(1:dim(X[Y == ylab,])[1], size=dim(X[Y==ylab,])[1], replace=FALSE))
  }
  xmax <- max(abs(X[,1])); xmin <- -xmax
  ymax <- max(abs(X[,2])); ymin <- -ymax
  X <- X[reorder,]; Y <- Y[reorder]
  df.dat <- data.frame(d1=X[, 1], d2=X[, 2], class=as.factor(Y))
...
sim_d_fill <- function(mus, Sigmas, outlier=NULL, title="", ylab="Magnitude", xlab="Dimension", ndim=10, nbreaks=4) {
  mu.dat <- melt(mus)
  d <- dim(mus)[1]
...
for (i in 1:length(sims)) {
  ...
  sim.fill[[counter]] <- sim_d_fill(simn$mus, simn$Sigmas, outlier=simn$robust, title=sim_names[i])
  counter <- counter + 1
}
```
...
resultsm.means <- aggregate(lhat ~ sim + alg + r + lhat, data=resultsm, FUN=nan.mean)
results.means <- aggregate(lhat ~ sim + alg + r + lhat, data = results, FUN = nan.mean)
results.means <- rbind(results.means, resultsm.means)
algs <-  c("LOL", "RLOL", "QOQ", "PLS", "CCA", "LDA", "PCA", "ROAD", "LASSO")
acols <- c("#f41711", "#f41711", "#f41711", "#94d6c9", "#87c6cc", "#99b4c6", "#020202", "#666666", "#666666")
linestyle <- c(1,3,3,1,3,1,1,3,1)
linestyle <- c("solid", "dotted", "dashed", "dotted", "solid", "dotted", "solid", "solid", "dotted")
names(acols) <- algs
...
fsize=12
sim_leg.plot <- ggplot(data=subset(results.means, alg != "RP"), aes(x=r, y=lhat, color=alg, linetype=alg)) +
  geom_line(size=1.2) +
  scale_color_manual(values=acols, guide=guide_legend(nrow=3, byrow=TRUE), name="Algorithm") +
  scale_linetype_manual(values=linestyle, guide=guide_legend(nrow=3, byrow=TRUE), name="Algorithm") +
  theme(legend.position="bottom", axis.title.y=element_text(size=fsize)) +
  theme_bw()
...
simfill_leg.plot <- ggplot(dummydat, aes(x=Dimension, y=Magnitude, color=Class, fill=Class)) +
  ...
  scale_color_manual(values=mcols, guide=guide_legend(nrow=1, byrow=TRUE)) +
  scale_fill_manual(values=mcols, guide=guide_legend(nrow=1, byrow=TRUE))
...
grid_sim <- grid.arrange(grid.arrange(grobs=sim_plots, nrow=nsim), sim_leg, nrow=2, heights=c(.95, .07),
                         top=textGrob("Misclassification Rate\n(n=100)", gp=gpar(fontsize=tfonts, face="bold")))
...
grid_simfill <- grid.arrange(grid.arrange(grobs=sim.fill, nrow=nsim), simfill_leg, nrow=2, heights=c(.95, .07),
...
grid.arrange(grid_sim, grid_simfill, ncol=2, widths=c(0.35, 0.35))
```
