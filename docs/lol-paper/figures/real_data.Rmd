...
make_marginal_2d <- function(data, xlims, ylims, plot.title="", xl="", yl="", leg.title="",
  ...
  top <- ggplot(data, aes(x=rstar.norm, y=..scaled.., color=alg, linetype=alg)) +
    ...
    theme(legend.position=NaN,
        ...
        axis.ticks.x=element_blank(),
        axis.text.x=element_blank())
  return(arrangeGrob(top, leg, center + theme(legend.position=NaN), right, ncol=2, nrow=2, widths=c(4,1.2), heights=c(2,4)))
}
...
fig.5 <- grid.arrange(scatter,
                      arrangeGrob(dset.mnist + ggtitle("(B) MNIST dataset, n=70,000, d=784") + xlab("Embedding Dimension") + ylab("Misclassification Rate"),
                                  dset.mrn + ggtitle("(C) Sex dataset, n=114, d > 500,000,000") +  xlab("") + ylab(""),
