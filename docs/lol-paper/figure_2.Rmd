...
# LDA projection plot
run_sim <- function(inps, func, ylab="", title="", r=20) {
  result <- do.call(func, c(inps, list(r=r)))
  
  liney <- lda(result$Xr, inps$Y)
  result <- predict(liney, result$Xr)
  data <- data.frame(x1=result$x[,1], y=inps$Y)
  data$y <- factor(data$y)
...
alg_names <- c("PCA", "PCA'", "LR-CCA", "LOL", "Bayes Optimal")
