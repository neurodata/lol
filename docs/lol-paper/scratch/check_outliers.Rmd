...
```{r}
data <- lol.sims.rtrunk(n, d)
X = data$X; Y = data$Y

# generate outliers
data.outlier <- lolR:::lol.sims.sim_gmm(mus=data$mus,  Sigmas=10*data$Sigmas, n=n, priors=data$priors)
...
data <- data.frame(x1=newXr[,1], x2=newXr[,2], y=Y)
data$y <- factor(data$y)
ggplot(data, aes(x=x1, y=x2, color=y)) +
  geom_point(alpha=0.6) +
  xlab("x1") +
...
```{r}
result <- lol.project.lol(X, Y, r, robust=TRUE)
