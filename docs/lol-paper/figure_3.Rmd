...
sim_cov_plot <- function(Sigmas, title="", ylab="Dimension", xlab="Dimension", ndim=10, legend.name="cov") {
  Sigma <- apply(Sigmas, c(1, 2), sum)
  plot_cov <- gs.plot.plot_matrix(Sigma[1:ndim, 1:ndim], title=title, xlabel=xlab,
...
results[results$lhat > 0.5,]$lhat <- (1 - results[results$lhat > 0.5,]$lhat)
