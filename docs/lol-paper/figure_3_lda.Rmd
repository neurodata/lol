...
plot_sim_lhats <- function(data, cols, pt.dat, title="", by=10, from=10, ylab=TeX("$\\hat{L}$"),
  ...
  plot_sims <- ggplot(data, aes(x=r, y=lhat, group=alg, color=alg)) +
    ...
    theme(legend.position="bottom", axis.title.y=element_text(size=fsize)) +#, title.position="top", title.hjust = .5)) +
...
names(acols) <- algs
linestyle <- c("solid", "dashed", "solid", "solid", "solid", "solid", "solid")
...
for (i in 1:length(sim_names)) {
  ...
  sim_plots[[counter]] <- plot_sim_lhats(data_sub, acols, pt.dat, ylab=sim, from=sim_min[i], by=by[i])
  counter <- counter + 1
}
```
