...
sim_cov_plot <- function(Sigmas, mus, priors, title="", yl="Dimension", xl="Dimension", ndim=10,
                         nbreaks=4, legend.name=TeX("norm $\\hat{\\Sigma}$")) {
  ...
  plot_cov <- ggplot(sdat, aes(x=Var1, y=Var2, fill=value)) +
    ...
    scale_fill_gradientn(name=legend.name, colours=rev(c("#fcfbfd", "#9e9ac8", "#3f007d")),
                         limits=c(0, 1), breaks=c(0.0, 0.5, 1.0))#,
                         #guide=guide_colorbar(title.position="top", title.hjust = .5, barheight=.75))
}

mcols <- c("#e41a1c", "#377eb8", "#4daf4a")
names(mcols) <- c(1,2,3)
sim_mean_plot <- function(mus, title="", ylab="Magnitude", xlab="Dimension", ndim=10, nbreaks=4) {
  ...
  lims <- c(floor(10*min(dat$Magnitude))/10, ceiling(10*max(dat$Magnitude))/10)
  breaks= c(lims[1], round((lims[1] + lims[2])/2), lims[2])
...
plot_sim_lhats <- function(data, cols, title="", ylab=TeX("$\\hat{L}$"), xlab="Embedded Dimensions", fsize=12) {
  ...
  xbreaks <- c(xlims[1], round((xlims[1] + xlims[2])/2), xlims[2])
  plot_sims <- ggplot(data, aes(x=r, y=lhat, group=alg, color=alg)) +
    ...
    scale_color_manual(values=cols, limits=names(cols),
                       guide=guide_legend(nrow=2, byrow=TRUE)) +#, title.position="top", title.hjust = .5)) +
...
maxr <- c(90, 30, 30, 30)
minr <- 1
mats <- list(toep, tr2, tr3, ft)
sim_name <- c("Toeplitz", "Trunk-2", "Trunk-3", "Fat-Tails (D=1000)")

interest <- list(c("ROAD"), c("ROAD"), c("LASSO"), c("ROAD"))
key <- c("ROAD", "lasso")
...
acols <- c("#006400", "#4daf4a", "#e41a1c", "#6b6b6b", "#d95f02", "#984EA3", "#00ffff")
names(acols) <- c("LOL", "QOQ", "CCA", "ROAD", "LASSO", "PCA", "cPCA")

sim_plots <- list()
counter <- 1
for (sim in sim_names) {
  data_sub <- results.means[results.means$sim == sim,]
  sim_plots[[counter]] <- plot_sim_lhats(data_sub, ylab=sim, cols=acols)
  counter <- counter + 1
}
```
...
grid.arrange(grid_sim, grid_mean, grid_cov, ncol=3, widths=c(0.35, 0.35, 0.23))
```
